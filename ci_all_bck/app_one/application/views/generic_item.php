...
<html lang="en">
  ...
  <body>
    ...
    <div class="form-group col-md-6" id="">
      <label class="form-check-label" for="drug_type_id">Drug Type</label>
      <?php foreach($drug_type as $record){ ?>
      <select class="form-control" name="drug_type_id" id="drug_type_id">
      <option value="<?php echo $record->drug_type_id; ?>" <?php if($input_data->drug_type_id == $record->drug_type_id) echo 'selected'; ?>><?php echo $record->drug_type; ?></option>
      <?php } ?>
      </select>
    </div>
  </div>
