...
<html lang="en">
  ...
  <body>
    ...
    <div class="form-group col-md-6" id="">
      <label class="form-check-label" for="call_id">Callsid</label>
      <?php foreach($helpline_call as $record){ ?>
      <select class="form-control" name="call_id" id="call_id">
      <option value="<?php echo $record->call_id; ?>" <?php if($input_data->call_id == $record->call_id) echo 'selected'; ?>><?php echo $record->callsid; ?></option>
      <?php } ?>
      </select>
    </div>
  </div>
