...
<html lang="en">
  ...
  <div class="form-row" id="">
    ...
    <div class="form-group col-md-6" id="">
      <label class="form-check-label" for="equipment_id">Make</label>
      <?php foreach($equipment as $record){ ?>
      <select class="form-control" name="equipment_id" id="equipment_id">
      <option value="<?php echo $record->equipment_id; ?>" <?php if($input_data->equipment_id == $record->equipment_id) echo 'selected'; ?>><?php echo $record->make; ?></option>
      <?php } ?>
      </select>
    </div>
  </div>
